...
const canAddMoreDocs = computed(
	() => authStore.docs_count < 10 || authStore.subs_status
);
...
const {
	data: filesResponse,
	error,
	refresh: doFetchRefresh,
} = await useFetch(
	`${config.public.apiBase}/files/${account_unique_id}/${id}`,
	{
		method: 'GET',
		headers: {
...
if (error.value) {
	...
	toast.add({
		title: 'Error',
		description: `Could not fetch files: ${
...
const filesWithDisplayNames = computed(() => {
	...
	return filesResponse.value.files.map((file) => {
		...
		if (lastUnderscoreIndex !== -1) {
			const potentialIdPart = originalFileName.substring(
				lastUnderscoreIndex + 1
			);
			// Regex to check for common ID patterns like _hexchars.pdf or _number.pdf
			if (
				/^[a-f0-9]+(\.pdf|\.txt|\.docx|\.doc|\.md)$/i.test(
					potentialIdPart
				) ||
				/^[0-9]+(\.pdf|\.txt|\.docx|\.doc|\.md)$/i.test(potentialIdPart)
			) {
				displayNameInProgress = originalFileName.substring(
					0,
					lastUnderscoreIndex
				);
			}
		...
		displayNameInProgress = displayNameInProgress.replace(
			/\.(pdf|txt|docx|doc|md)$/i,
			''
		);
		...
		const isProcessed =
			file.already_processed_to_source_data === true ||
			String(file.already_processed_to_source_data).toLowerCase() ===
				'true';
...
const handleDeleteFileConfirmed = async () => {
	...
	try {
		const response = await $fetch(
			`${config.public.apiBase}/files/${account_unique_id}/${fileBeingDeleted.id}`,
			{
				method: 'DELETE',
				headers: {
		...
		toast.add({
			title: 'File Deleted',
			description: `File "${fileBeingDeleted.file_name}" has been deleted.`,
			color: 'green',
		});
		...
		const errorMessage =
			err.data?.detail || err.message || 'Could not delete file.';
			...
			if (
				...
				(value.toLowerCase().includes(q.value.toLowerCase()) ||
					...
					(file.file_name &&
						file.file_name
							.toLowerCase()
							.includes(q.value.toLowerCase()))) // Search original too
			) {
				return true;
			}
			return false;
